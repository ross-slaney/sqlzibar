...
That failure is deliberate. Cookie authentication has to run before the surface establishes its bearer identity, and a guard inside a branch is rejected rather than silently skipped.
...
The Notes sample in the repository shows the full shape: browser OIDC sign-in, a protected API, and MCP tools sharing one FGA-enforcing service, with two-user isolation, revocation, and logout exercised against real SQL Server and PostgreSQL. See [MCP server](/docs/authserver/mcp-server).
...
- **Place `UseSqlOSSurfaceProtection()`** after `UseAuthentication()` and before `UseAuthorization()` if you declare `Api` or `Mcp` and your host registers ASP.NET authentication. Startup tells you if it is missing or in a branch.
