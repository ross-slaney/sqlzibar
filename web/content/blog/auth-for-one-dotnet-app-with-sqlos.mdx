...
SqlOS maps its routes at startup and derives the first-party PKCE client, callback, API audience, and protection. Your browser application uses its standard OIDC library with the registered callback, through either hosted AuthPage or headless UI. If this same process registers ASP.NET authentication, place `UseSqlOSSurfaceProtection()` after `UseAuthentication()` and before `UseAuthorization()`. See the [complete README browser example](https://github.com/ross-slaney/sqlos#one-application-sign-in-and-call-your-api).
