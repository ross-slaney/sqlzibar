...
`AddSqlOS` installs the guard for declared surfaces ahead of the application's pipeline; nothing is placed by application code. Requests under a declared surface that carry no token or a token for another audience receive HTTP `401` with a Bearer challenge that names the surface's `realm` and `resource_metadata` URL, before any application middleware, branch, or endpoint runs. The validated token is available through `HttpContext.GetSqlOSValidatedToken()` and `HttpContext.User`; `RequireSqlOSAccessToken` on a nested group reuses it and only adds scope enforcement. A CORS preflight is passed to the host's `UseCors` when CORS services are registered; otherwise `OPTIONS` is challenged like any other request. Surfaces match the request path as received, so declare the paths clients request if the application applies `UsePathBase` or rewrites.
