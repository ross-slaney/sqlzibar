...
| `Audience`, `ClientId`, `RedirectPath`, `RedirectUris`, `AllowedScopes` | | Client overrides. `Audience` defaults to `{Origin}{Api}` when `Api` is set. |
...
## UseSqlOSSurfaceProtection

**Namespace:** `SqlOS.Extensions`

Places the declared API/MCP guard in the root application pipeline. Call after exception handling, rewriting, `UsePathBase`, routing, CORS, and authentication, and before authorization or any middleware that serves protected content. Surface paths are application-relative after `UsePathBase`. Repeated calls are idempotent; placing the guard inside a branch is rejected.
...
Without explicit placement, a bearer-only host gets an early guard before application middleware. Hosts registering ASP.NET authentication must place it explicitly or startup fails: cookie authentication must run before the surface's bearer identity is established. Policies inside the surface should use the current principal, not select a different authentication scheme. Configured CORS may answer preflights before the guard; `OPTIONS` alone does not bypass it.
